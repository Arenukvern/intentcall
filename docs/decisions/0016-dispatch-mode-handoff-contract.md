...
publication is curated separately with `includeInShortcuts`; not every tool is
...
- Manifest authors need a separate field for shortcut publication.
- Existing app authors who expected every tool to appear in App Shortcuts must
  opt in with `includeInShortcuts`.
