...
Generated Apple `AppIntent` structs land in the app `Runner` target; they
`import intentcall_platform_apple` and call `IntentCallNativeBridge.enqueue` —
handoff queue and deep-link open logic live in the plugin, not duplicated per app.
...
|---|---|---|
| Apple App Intents / Shortcuts | `intentcall_platform_sync` emitters → `Runner/Generated/`; runtime facade + stores in `intentcall_platform_apple` | `platform sync --check` (drift); `check_apple_runner_compile.sh` in mcp_flutter (compile); AppIntentsTesting / signed app for runtime |
...
|---|---|---|
| **mcp_flutter maintainers** (sibling dogfood) | Path deps to `../agentkit/packages/intentcall_*` | `make check-contracts` (includes Apple compile gate), `make dogfood-eval-static` |
...
| **macOS** | Runtime — plugin facade compile + host drain + validate | `bash tool/contracts/check_apple_runner_compile.sh`, `make showcase`, `make macos-validate-runtime` |
