...
| `intentcall_platform_sync` — native/web emitters, `PlatformSync`, `ManifestExporter` | Any production app serving end users |
| `intentcall_platform` — Flutter runtime host / federated umbrella (not emitters) | |
| `intentcall_codegen` — optional `@AgentTool` code generation | |
| `intentcall_testing` — contract / invoke test helpers | |
| `intentcall_gemma` — surface adapter | |
| `intentcall_platform_apple` / `intentcall_platform_android` — federated native impls (endorsed by umbrella) | |
...
- [Who is this for?](/start_here/audiences) — in-app, external MCP agents, OS surfaces, mcp_flutter, Gemma
