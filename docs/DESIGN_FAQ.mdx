...
**Q: Why were there separate `intentcall_apple` / `intentcall_android` packages, and what replaced them?**
A: Those packages were sparse-manifest generators for platform-specific SDKs. They are **deprecated** (`publish_to: none`); canonical projection is `intentcall_platform_sync` (`PlatformSync` / `ManifestExporter`). Runtime native code will split into federated Flutter plugins per [ADR 0025](decisions/0025-platform-subset-federated-plugins.md). `intentcall_gemma` remains a separate surface adapter.
