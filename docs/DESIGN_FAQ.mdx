...
A: Not for the current public surface. `AgentRegistry.listDescriptors()` is the
neutral catalog snapshot and `AgentRegistry.events` is the neutral change feed.
Adapters consume that surface and project it into their transport. A named
gateway should appear only when it owns behavior beyond listing descriptors and
