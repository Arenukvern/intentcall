...
Set<AgentCallEntry> _buildDemoEntries({
  required final Map<String, Object?> Function({String? viewMode})
  applyControls,
  ...
  AgentCallEntry.tool(
    ...
    handler: (_) =>
        AgentResult.success(message: 'pong', data: const {'ok': true}),
  ),
};
...
void main() {
  ...
  test(
    'builder → invokeWire → AgentResult.envelope (design § Client DX)',
    () async {
      final entries = _buildDemoEntries(
        readSnapshot: () => const {'phase': 'playing'},
        applyControls: ({final viewMode}) => {
  ...
  test('entry set byName resolves registration descriptor', () {
    ...
    expect(
      entries.byName('a').toRegistration().descriptor.qualifiedName,
      'app_a',
    );
  });
}
