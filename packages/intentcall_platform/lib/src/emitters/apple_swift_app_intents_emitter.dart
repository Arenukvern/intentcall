...
final class AppleSwiftAppIntentsEmitter {
  ...
  String emit(final AgentManifest manifest) {
    ...
    for (final tool in manifest.tools) {
      ...
      buffer
        ..writeln(
          '    let invocationId = await IntentCallNativeBridge.enqueue(qualifiedName: "${escapeSwiftString(tool.qualifiedName)}", arguments: arguments)',
        )
        ..writeln(
    ...
    buffer
      ...
      ..writeln(
        r'    guard let scheme = fallbackScheme, let url = URL(string: "\(scheme)://invoke/\(qualifiedName)") else { return invocationId }',
      )
      ..writeln('    #if canImport(UIKit)')
