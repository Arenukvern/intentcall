...
(function intentcallWebMcpBootstrap(global) {
		...
		switch (type) {
			...
			case "boolean":
				if (typeof value !== "boolean")
					return validationError(path + " must be a boolean.");
				return null;
			case "object":
				if (
					typeof value !== "object" ||
			...
			case "list": {
				if (!Array.isArray(value))
					return validationError(path + " must be an list.");
				var listPath = path;
				if (
					listPath.length >= 2 &&
					listPath.charAt(0) === '"' &&
	...
	function validateObjectProperties(pathPrefix, schema, args) {
		...
		if (schema.additionalProperties === false) {
			for (var key in args) {
				if (Object.hasOwn(args, key) && !Object.hasOwn(properties, key)) {
		...
		if (required && required.length) {
			for (var r = 0; r < required.length; r += 1) {
				...
				if (!Object.hasOwn(args, name)) {
					...
					return validationError(
						'Missing required property "' + name + '"' + atReq + ".",
					);
				}
			}
		}
		for (var prop in properties) {
			...
			if (!Object.hasOwn(args, prop)) continue;
			var childPath = pathPrefix ? pathPrefix + "." + prop : prop;
			var propErr = validateValue(
				'"' + childPath + '"',
				properties[prop],
				args[prop],
			);
	...
	function validateArrayItems(path, schema, value) {
		...
		for (var pk in itemProperties) {
			if (Object.hasOwn(itemProperties, pk)) {
				hasProps = true;
				break;
			}
		}
	...
	function fetchInvoke(name, args) {
		...
		return global
			.fetch(invokePath + "?name=" + encodeURIComponent(name), {
				method: "POST",
			...
			modelContext.registerTool({
				...
				execute: (args) => {
					...
					if (typeof dart === "function") {
						return Promise.resolve(dart(tool.name, args || {})).then(
							(result) => {
